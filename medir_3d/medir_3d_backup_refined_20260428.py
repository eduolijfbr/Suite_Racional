        ...
        try:
            ...
            for bacia in selected_bacias:
                ...
                for vdata in vias_na_bacia:
                    ...
                    # Obter valores dos campos dinâmicos ou usar fixos
                    ...
                    cai_via_real = get_attr_val(campo_caimento, cai_fixo) # Pode vir do campo se selecionado

                    i_long = max(0.001, abs(vdata['zi'] - vdata['zf']) / vdata['L'])
                    q_cap = self.calcular_capacidade_sarjeta(n_real, lam_real, cai_fixo, i_long) # Mantém cai_fixo para o cálculo transversal se não houver campo específico
                    fator_e = 0.5 if i_long > 0.08 else (0.75 if i_long > 0.04 else 1.0)
                    q_engol_real = q_engol_fixo * fator_e

                    # Atributos comuns para as novas features
                    common_attrs = [id_val, cod_val, 0.0, 0.0, 0.0, n_real, lam_real, round(cai_via_real*100, 2)]
                    ...
                    for pv_d in pv_dists:
                        ...
                        f.setAttributes([id_val, cod_val, 0.0, 400.0, 1.0, n_real, lam_real, round(cai_via_real*100, 2)])
                    ...
                    for k in range(len(pontos_rede) - 1):
                        ...
                        f.setGeometry(seg_g)
                        f.setAttributes([id_val, cod_val, round(seg_len, 2), 400.0, round(max_c, 2), n_real, lam_real, round(cai_via_real*100, 2)])
